...
// Render-only virtual sink: no capture endpoint is registered. The user-mode engine reads
// the rendered 5.1 mix via WASAPI loopback on the render endpoint instead. (g_CaptureEndpoints
// stays defined/referenced by the install loop, which simply iterates zero times.)
